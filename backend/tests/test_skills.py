...
Each test uses the `client` fixture which wraps requests in a rolled-back
transaction, so tests are fully isolated and can run in any order.
"""
...
def test_create_skill(client):
    ...
    """
    resp = client.post("/api/skills/", json={"name": "CNC Operation", "category": "generic", "is_premium": False})
...
def test_list_skills(client):
    ...
    """
    client.post("/api/skills/", json={"name": "Welding", "category": "premium", "is_premium": True})
...
def test_get_skill(client):
    ...
    """
    create_resp = client.post("/api/skills/", json={"name": "Grinding", "category": "generic"})
...
def test_update_skill(client):
    ...
    """
    create_resp = client.post("/api/skills/", json={"name": "Painting", "category": "generic"})
    skill_id = create_resp.json()["id"]
    resp = client.patch(f"/api/skills/{skill_id}", json={"is_premium": True})
    assert resp.status_code == 200
...
def test_delete_skill(client):
    ...
    """
    create_resp = client.post("/api/skills/", json={"name": "Drilling", "category": "generic"})
    skill_id = create_resp.json()["id"]
