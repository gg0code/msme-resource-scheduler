...
```python
"""
backend/app/services/ai_service.py — V3.0

FILE PURPOSE
This file provides the AI Copilot service using Groq's LLaMA 3.3-70b model with function calling capabilities.
It serves as the narrative layer that translates raw business data into natural language insights for users.
Introduced in v3.0 and maintained in v4-dev branch, this file implements design principle #1: "Engine computes, 
AI only narrates" - the LLM never performs calculations or business logic, only provides human-readable 
explanations of data computed by other services. It sits between the FastAPI routers and the database models,
acting as an intelligent data interpreter that can answer complex business questions about jobs, costs, 
schedules, and resource utilization.

WHAT THIS FILE DOES — step by step
1. Defines utility functions for safe date parsing and normalization (safe_date_parse, as_date, safe_days)
2. Creates a Groq client factory function that pulls API key from application settings
3. Defines 13 AI tools/functions that the LLaMA model can call to retrieve business data
4. Each tool definition includes OpenAI-compatible function schemas with parameter descriptions
5. Tool implementations query the database for specific business metrics (revenue, costs, delays, availability)
6. Formats raw database results into structured data that the AI can narrate
7. Handles tenant scoping on all database queries to maintain security isolation
8. Provides comprehensive business intelligence across jobs, employees, machines, and financial metrics
9. Returns formatted responses that can be directly presented to users or further processed by AI
10. Supports date-based queries with intelligent defaults (current month/year when not specified)

KEY FUNCTIONS / CLASSES / COMPONENTS

Name         : safe_date_parse
Type         : function
Purpose      : Robustly parses date strings handling ISO format, relative terms like "today"/"tomorrow", 
               and invalid inputs. Prevents crashes when users provide ambiguous date inputs to AI queries.
Parameters   : check_str (Any) - the input to parse, can be string, dict, or None
               fallback (date) - default date to return if parsing fails
Returns      : date object representing the parsed date or fallback if parsing fails
Calls        : Built-in date.today(), date.fromisoformat(), timedelta()
DB/API       : None
Side effects : None

Name         : as_date
Type         : function
Purpose      : Normalizes datetime/date/string inputs to plain date objects for consistent date handling
               across the AI service. Handles the common case of database datetime fields needing date comparison.
Parameters   : d (Any) - input that should represent a date (datetime, date, string, or None)
Returns      : date object or None if conversion fails
Calls        : Built-in date.fromisoformat()
DB/API       : None
Side effects : None

Name         : safe_days
Type         : function
Purpose      : Safely computes the difference in days between two dates, handling datetime conversion and
               potential errors. Used for calculating job delays, duration, and date-based comparisons.
Parameters   : d1 (Any) - first date (can be datetime, date, or string)
               d2 (Any) - second date (can be datetime, date, or string)  
               default (int) - value to return if calculation fails (default 0)
Returns      : integer representing (d1 - d2).days or default value on error
Calls        : as_date() function defined in this file
DB/API       : None
Side effects : None

Name         : get_groq_client
Type         : function
Purpose      : Factory function that creates and configures a Groq API client using the API key from
               application settings. Centralizes client creation and validates that required credentials exist.
Parameters   : None
Returns      : Groq client instance configured with API key from settings.GROQ_API_KEY
Calls        : app.config.settings for retrieving GROQ_API_KEY
DB/API       : None (prepares for Groq API calls but doesn't make them)
Side effects : Raises ValueError if GROQ_API_KEY is not configured

Name         : TOOLS
Type         : constant list
Purpose      : Defines the complete set of 13 AI tools/functions that the LLaMA model can invoke to retrieve
               business data. Each tool has OpenAI-compatible function schema with name, description, and parameters.
               These tools cover revenue analysis, cost breakdowns, scheduling conflicts, resource availability,
               and comprehensive business intelligence queries.
Parameters   : N/A (constant definition)
Returns      : N/A (constant definition) 
Calls        : N/A (constant definition)
DB/API       : N/A (constant definition - individual tool implementations make DB queries)
Side effects : N/A (constant definition)

Name         : get_monthly_revenue (tool)
Type         : AI tool function definition
Purpose      : Tool that retrieves total order book value and revenue breakdown for a specified month/year.
               Handles questions about revenue, order value, income, and earnings with intelligent date defaults.
Parameters   : month (optional integer) - month number 1-12, defaults to current month
               year (optional integer) - 4-digit year, defaults to current year
Returns      : Revenue breakdown data structure for AI narration
Calls        : Database queries through SQLAlchemy ORM (Job model)
DB/API       : Queries Job table filtered by tenant_id and date range
Side effects : None (read-only database access)

Name         : get_raw_material_cost (tool)
Type         : AI tool function definition  
Purpose      : Tool that calculates total raw material costs across all jobs for a given month.
               Addresses questions about material spend, RM cost, and procurement expenses.
Parameters   : month (optional integer) - month number 1-12, defaults to current month
               year (optional integer) - 4-digit year, defaults to current year
Returns      : Raw material cost summary for the specified time period
Calls        : Database queries through SQLAlchemy ORM (Job model with raw_materials JSON field)
DB/API       : Queries Job table, processes raw_materials JSON field, filters by tenant_id
Side effects : None (read-only database access)

Name         : get_delayed_jobs (tool)
Type         : AI tool function definition
Purpose      : Tool that identifies all jobs that are delayed, behind schedule, not started, or at risk.
               Critical for operational oversight and identifying scheduling problems that need immediate attention.
Parameters   : None
Returns      : List of delayed/at-risk jobs with delay metrics and reasons
Calls        : Database queries through SQLAlchemy ORM (Job, JobStep models)
DB/API       : Complex queries across Job and JobStep tables with date comparisons, filtered by tenant_id
Side effects : None (read-only database access)

Name         : get_machine_availability (tool)  
Type         : AI tool function definition
Purpose      : Tool that determines which machines are free, busy, or idle on a specific date.
               Essential for resource planning and answering capacity questions.
Parameters   : check_date (optional string) - date in YYYY-MM-DD format, defaults to today
Returns      : Machine availability status breakdown with utilization details
Calls        : Database queries through SQLAlchemy ORM (Machine, JobAssignment models)
DB/API       : Joins Machine and JobAssignment tables with date filtering, tenant_id scoping
Side effects : None (read-only database access)

Name         : get_employee_availability (tool)
Type         : AI tool function definition  
Purpose      : Tool that shows which employees are available, free, or busy on a specific date.
               Supports workforce planning and answers staffing availability questions
"""
