"""
resource_availability.py  (backend/app/schemas/)
Pydantic response schemas for the GET /api/jobs/{job_id}/resource-availability endpoint.
Defines the shape of employee availability, machine availability, and the top-level response.
These are NOT database models — they are API response validators only.
Added in v3.9.4. The materials list is intentionally empty here and will be populated in v3.9.5
when inventory/stock tracking is introduced, so the frontend contract does not need to change then.
"""
