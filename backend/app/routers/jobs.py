...
```python
"""
ZetaOps Copilot - Job Management Router (v4-dev)

FILE PURPOSE
This is the FastAPI router module that handles all HTTP endpoints for job management 
in the ZetaOps Copilot scheduling system. It provides CRUD operations for jobs, 
automatic conflict detection, cost overrun analysis, and job timer functionality. 
This file was introduced in v1.0 and significantly enhanced in v1.2 to include 
automatic conflict detection via the availability engine. It sits in the API layer 
between the React frontend and the SQLAlchemy models, handling all /api/jobs/* routes.

WHAT THIS FILE DOES — step by step
1. Imports all necessary FastAPI components, SQLAlchemy ORM tools, Pydantic schemas, 
   and internal ZetaOps modules for authentication, database access, and business logic
2. Defines Pydantic schemas (RawMaterial, SkillReqIn, JobCreate, JobUpdate, TimerAction) 
   for request/response validation and type safety
3. Implements helper function _refresh_conflict() that calls the availability engine 
   to compute and persist has_conflict flags on jobs automatically
4. Implements helper function _cost_overrun() that analyzes whether actual job hours 
   exceed estimated hours by more than 10% and returns overrun details
5. Implements job_to_dict() serializer that converts SQLAlchemy Job models into 
   JSON-safe dictionaries with all relationships and computed fields
6. Defines PRIORITY_RANK mapping for job priority ordering in scheduling algorithms
7. Registers FastAPI router endpoints for tenant info, job listing, creation, 
   retrieval, and updates with proper authentication and authorization

KEY FUNCTIONS / CLASSES / COMPONENTS

Name         : RawMaterial
Type         : Pydantic BaseModel class  
Purpose      : Validates and serializes raw material data for jobs. Ensures each 
               material has a name, quantity, unit (defaults to "pcs"), and unit_cost 
               for accurate job costing and inventory tracking.
Parameters   : name (str), quantity (float), unit (str, default "pcs"), unit_cost (float)
Returns      : Pydantic model instance for JSON serialization
Calls        : None (pure data class)
DB/API       : None
Side effects : None

Name         : SkillReqIn  
Type         : Pydantic BaseModel class
Purpose      : Validates skill requirement input when creating or updating jobs. 
               Specifies which skills are needed, minimum competency levels, and 
               how many employees with those skills are required for the job.
Parameters   : skill_id (int), min_skill_level (str, default "Generic"), employees_required (int, default 1)
Returns      : Pydantic model instance for JSON serialization
Calls        : None (pure data class)
DB/API       : None
Side effects : None
...
Name         : JobCreate
Type         : Pydantic BaseModel class
Purpose      : Validates all input data when creating new jobs via POST /api/jobs/. 
               Includes required fields like name and dates, plus optional fields for 
               customer info, financials, scheduling preferences, and skill requirements.
Parameters   : name (str), start_date (str), end_date (str), plus 20+ optional fields including customer, priority, raw_materials, skill_requirements, job_type, quantity
Returns      : Pydantic model instance for JSON serialization
Calls        : None (pure data class)
DB/API       : None
Side effects : None
...
Name         : JobUpdate
Type         : Pydantic BaseModel class
Purpose      : Validates partial job updates via PATCH /api/jobs/{id}. All fields are 
               optional since this handles partial updates, allowing clients to send 
               only the fields they want to change without affecting others.
Parameters   : All JobCreate fields but made optional with Optional[type] = None
Returns      : Pydantic model instance for JSON serialization
Calls        : None (pure data class)
DB/API       : None  
Side effects : None
...
Name         : TimerAction
Type         : Pydantic BaseModel class
Purpose      : Validates job timer actions for start/pause/resume/end operations. 
               Used by timer endpoints to ensure only valid timer state transitions 
               are allowed when tracking actual job execution time.
Parameters   : action (str) - must be "start", "pause", "resume", or "end"
Returns      : Pydantic model instance for JSON serialization
Calls        : None (pure data class)
DB/API       : None
Side effects : None

Name         : _refresh_conflict
Type         : helper function
Purpose      : Computes scheduling conflicts for a job using the availability engine 
               and persists the has_conflict flag to the database. This is called 
               automatically after job creation or updates to keep conflict status current.
Parameters   : db (Session) - SQLAlchemy database session, job (Job) - job model instance, tenant_id (int) - tenant scope for security
Returns      : AvailabilityResult object from availability engine or None on error
Calls        : app.services.availability_engine.check_availability()
DB/API       : Updates job.has_conflict field in database
Side effects : Modifies job.has_conflict flag, never raises exceptions to avoid breaking saves

Name         : _cost_overrun  
Type         : helper function
Purpose      : Analyzes whether actual job hours significantly exceed estimated hours 
               (>10% overrun) and returns detailed overrun information for cost management. 
               Helps identify jobs that are consuming more resources than planned.
Parameters   : job (Job) - job model instance with actual_hours, start_date, end_date, estimated_hours_per_day
Returns      : dict with overrun_pct, actual_hours, estimated_hours or None if no significant overrun
Calls        : None (pure calculation)
DB/API       : None (read-only analysis)
Side effects : None

Name         : job_to_dict
Type         : serializer function
Purpose      : Converts SQLAlchemy Job model instances into JSON-safe dictionaries 
               for API responses. Handles date serialization, relationship loading, 
               and computes derived fields like cost_overrun status and timer information.
Parameters   : job (Job) - SQLAlchemy Job model instance with loaded relationships
Returns      : dict containing all job fields, relationships, and computed properties ready for JSON serialization
Calls        : _cost_overrun() helper function
DB/API       : None (pure serialization)
Side effects : None

Name         : get_tenant_info
Type         : FastAPI GET endpoint
Purpose      : Returns tenant information including the job_id_prefix setting which 
               is used by the frontend to generate properly formatted job IDs. 
               Provides tenant context needed for job management operations.
Parameters   : db (Session) - injected database session, current_user (User) - authenticated user from JWT
Returns      : dict with tenant id, name, slug, plan, job_id_prefix
Calls        : app.core.dependencies.get_current_user(), app.database.get_db()
DB/API       : Queries Tenant table filtered by current_user.tenant_id
Side effects : None

Name         : list_jobs
Type         : FastAPI GET endpoint  
Purpose      : Returns all jobs for the authenticated user's tenant with full relationship 
               data loaded. Implements tenant scoping for security and orders by start_date. 
               Used by frontend to populate job lists and dashboards.
Parameters   : db (Session) - injected database session, current_user (User) - authenticated user from JWT
Returns      : List of job dictionaries with full
"""
