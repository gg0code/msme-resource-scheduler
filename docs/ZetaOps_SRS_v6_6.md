...
## 6.28.6 Channel-Decision Send Helper (v6.3.22)

**Strategic intent.** Meta's WhatsApp Business API enforces a 24-hour conversation window. Outside that window only pre-approved HSM templates may be sent — free-form text is rejected at the wire. Through v6.3.21 every outbound dispatcher (`whatsapp_alerts._send_alert`, the v6.3.4 push briefing dispatcher, the v6.3.5 invite welcome, the v6.3.16 Day-7 gate, the v6.3.19.1 delay + conflict dispatchers) sent free-form messages. That worked in mock mode but would fail silently in production for any recipient who hadn't messaged the bot in the previous 24 hours. v6.3.21 part 2 shipped the static template registry (`resolve_template`, `EVENT_ROUTING`); v6.3.22 wires the runtime channel decision that selects between the two transports.

**Module.** `backend/app/services/whatsapp_send_helper.py` ships `send_with_window_decision()` as the single outbound entry point. The function reads `phone_tenant_map.last_seen_at`, branches at the 24h boundary, and either calls the existing `_send_whatsapp_message` (in-window free-form) or `_post_template_to_meta` (out-of-window HSM template). `_post_template_to_meta` builds the Meta-compliant template POST body by splitting `meta_params` into HEADER-component and BODY-component arrays — the split is driven by counting `{{n}}` placeholders in `META_TEMPLATES["components"]["HEADER"].text`. The helper never raises; transport errors come back as `SendOutcome(success=False, error=...)` so dispatchers retain their existing top-level try/except shape as defence in depth.

**Inbound-window source.** Reuses `phone_tenant_map.last_seen_at` (migration 017). Audit during the v6.3.22 design pass confirmed `last_seen_at` is written by exactly one production caller — `resolve_identity()` in `app/services/whatsapp_identity.py:156-160` — and `resolve_identity()` has three callers, all on inbound paths (`app/routers/whatsapp.py:388/678/726`). No outbound send mutates the column. The v6.3.22 doc draft proposed a new `last_inbound_at` column via migration 035; that was dropped after the audit. Migration head stays at 034.

**Audit vocabulary.** Five new `event_type`s under the dotted `whatsapp.` namespace, each written via the same `_write_event` helper:

| Event | When it fires |
| --- | --- |
| `whatsapp.freeform_sent` | In-window send completed (success or transport failure) |
| `whatsapp.template_sent` | Out-of-window template send completed; carries `meta_name`, `meta_language`, `meta_params`, `used_fallback`, `wamid`, `rendered_text` |
| `whatsapp.template_param_mismatch` | Caller passed wrong arg count; no wire call made |
| `whatsapp.window_expired_no_template` | Unknown event for an out-of-window recipient; no wire call made |
| `whatsapp.template_rejected` | Real-mode Meta returned 4xx/5xx after a template POST |

Every payload carries `phone`, `event`, `language`, `alert_type`, `within_window`, `last_seen_at` as the base shape, with branch-specific keys layered on. `entity_type='whatsapp_send'`, `source='system'`. The dispatcher's own per-tenant aggregate events (`push.morning_sent`, `push.delay_sent`, etc.) continue to fire alongside the helper's per-recipient events — the two granularities coexist intentionally.

**Mock-mode breadcrumb.** A new `[MOCK TEMPLATE]` log line is emitted from `_post_template_to_meta` in mock mode, carrying `event_template=`, `language=`, and `params=`. Sits alongside the pre-existing `[MOCK ALERT]` (from `whatsapp_alerts._send_alert`, retained on every callsite that emits it today) and `[MOCK SEND]` (from `_send_whatsapp_message`) lines. Existing log-grep tests for the earlier breadcrumbs continue to pass.

**Dispatcher refactors.** Five send paths route through `send_with_window_decision` in v6.3.22:
- `consolidated_briefing._dispatch_one` (morning + evening, sharing one code path) — uses `event="morning_briefing"` for both today; the evening template proper is on the v6.4 backlog per CHANGELOG `[v6.3.19]`.
- `consolidated_briefing.dispatch_delay_alert` — `event="job_ending_soon"` (delay reuses the same template per v6.3.19.1 cutover note; the dedicated `zetaops_job_delayed` template is on the v6.4 backlog).
- `consolidated_briefing.dispatch_conflict_alert` — `event="job_conflict_alert"`.
- `team_invite_whatsapp.send_invite_welcome` — `event="invite_team_member"`; invite recipients have `last_seen_at IS NULL` by construction, so the helper always picks the template path here.

`_render_morning_message` now returns `(rendered_text, args_tuple)`. The dispatcher passes `rendered_text` as `free_form_text` and `args_tuple` as `args`, so the same positional values drive both the in-window `.format()` render and the out-of-window `resolve_template` render.

**Deferred to v6.4.0 (and later).** Day-7 insight dispatcher in `day7_insight._default_send` stays on the free-form path: each `SignalCandidate` does not yet carry the six positional args that `zetaops_owner_day7_insight` expects (week_label, owner_first_name, pattern, evidence_1, evidence_2, action), and the template is still `pending_meta_approval` upstream. The five `_send_alert` callsites in `whatsapp_alerts.py` (machine_breakdown, manager_checkin, owner_briefing_from_checkin, legacy briefings dispatcher, send_morning_briefing fallback) likewise stay on free-form — their inputs are pre-rendered strings without args plumbing. Both deferrals are mock-mode safe today (`WHATSAPP_MOCK_MODE=True` does not exercise the Meta wire) and will be picked up alongside the v6.4.0 engagement-ladder dispatcher rewrite.

**Acceptance criteria.**

(6.28.6-AC1) `last_seen_at` continues to be populated by `resolve_identity()` on every inbound webhook write; no outbound path mutates it. (6.28.6-AC2) `send_with_window_decision` picks the free-form path when `now - last_seen_at < 24h`, and the template path when `last_seen_at IS NULL` or `now - last_seen_at >= 24h` — covered by `test_ac1_in_window_uses_freeform_path`, `test_ac2_outside_window_uses_template_path`, `test_ac3_never_seen_uses_template_path`. (6.28.6-AC3) The template registry from v6.3.21 covers every event the v6.3.22 dispatchers pass — `morning_briefing`, `job_ending_soon`, `job_conflict_alert`, `invite_team_member` — plus `owner_day7_insight` / `engagement_give_up_nudge` (wired for v6.4). (6.28.6-AC4) Param-count mismatch raises `ArgCountMismatchError` inside `resolve_template`, the helper writes `whatsapp.template_param_mismatch`, and no wire call is made — `test_ac5_param_mismatch_drops_with_audit`. (6.28.6-AC5) Unknown event for an out-of-window recipient writes `whatsapp.window_expired_no_template` and drops silently — `test_ac4_unknown_event_drops_silently_with_audit`. (6.28.6-AC6) Five dispatcher callsites route through the helper (4 in `consolidated_briefing.py` + `send_invite_welcome`); Day-7 and the five `_send_alert` callers are documented as deferred in the dispatcher comments. (6.28.6-AC7) Mock mode emits the `[MOCK TEMPLATE]` breadcrumb carrying `event_template=`, `language=`, `params=` — `test_ac6_mock_template_log_format`. (6.28.6-AC8) The free-form text the dispatcher renders matches `ResolvedTemplate.rendered_text` byte-for-byte for the same args — `test_ac7_content_equivalence_freeform_vs_template_render`.

**Out of scope (deferred).** Per-recipient locale routing via a future `User.language_preference` column (today every dispatcher passes `language="en_US"` or `language="hi"` based on the tenant-level locale). Meta v2 conditional `{flag_section}` / `{next_step_section}` template re-submission (closes the §23 tone exception around the placeholder flag in the morning briefing). Dedicated `zetaops_job_delayed` template registration (today `dispatch_delay_alert` reuses `zetaops_job_ending_soon`). `JOB_CONFLICT_ALERT_HI` Devanagari HEADER prefix (Hindi conflict-alert is BODY-only today). Day-7 insight + engagement ladder dispatcher rewrite to plumb 6-arg `SignalCandidate` tuples through the helper.

**Risks specific to v6.3.22.** (a) The dispatcher test fixture `mock_meta_sender` patched `consolidated_briefing._send_whatsapp_message`; with the helper in place the dispatcher no longer holds that reference. Mitigated by rebinding the patch target to `whatsapp_send_helper._send_whatsapp_message` and seeding `_make_phone_map` rows with a recent `last_seen_at` so the existing assertions on rendered text stay correct. (b) The five unrefactored `_send_alert` callers will silently fall back to free-form even when production cutover lands and a recipient is outside the 24h window. Mitigated by the fact that `WHATSAPP_MOCK_MODE=True` is still the default, so neither real-mode failure mode is reachable today; the v6.4.0 plan tracks the explicit refactor and the operational risk is bounded.

## 6.29 Brand Asset Library (v6.3.23)

**Strategic intent.** Outbound HSM templates went out as plain-text headers through v6.3.22, which read as generic Meta system messages. v6.3.23 introduces eight branded IMAGE-header assets — one per outbound category — so every approved template carries the ZetaOps brand chrome at the top of the message. The asset is part of the Meta template registration (HEADER type = IMAGE), not a per-send choice: when Meta approves the IMAGE-header re-submission for a (template, language) pair, every subsequent send for that pair includes the image automatically.

**Scope.** Eight categories shipped this release: `morning_briefing` (#185FA5, sun, "MORNING BRIEFING"), `evening_summary` (#3C3489, moon, "EVENING SUMMARY"), `compliance_reminder` (#BA7517, document, "COMPLIANCE"), `savings_summary` (#0F6E56, trend-up, "SAVINGS"), `conflict_alert` (#A32D2D, alert-triangle, "ALERT"), `team_invite` (#534AB7, user-plus, "INVITE"), `material_estimate` (#D85A30, calculator, "ESTIMATE"), `day7_first_insight` (#1D9E75, check, "DAY 7"). Two categories deferred to v6.3.24: `manager_checkin`, `einvoice_ready`. No new migration; head stays at **034**.

**Assets.** Committed at `backend/assets/whatsapp_headers/<key>.png` (8 files, 640×335 px PNG, each under 9 KB). SVG sources under `_source/<key>.svg` for designer edits. Generator at `backend/scripts/generate_brand_headers.py` renders both formats from a single in-module `SPEC` table using Pillow (no cairo dependency). Pillow added explicitly to `backend/requirements.txt`. Asset README at `backend/assets/whatsapp_headers/README.md` documents the design tokens and the regeneration workflow.

**Registry.** New module `backend/app/services/whatsapp_template_assets.py` ships `REGISTRY: dict[str, BrandAsset]` (8 entries keyed by brand category) plus `_META_NAME_TO_BRAND_KEY` (reverse map from Meta template name → brand key, used by the helper at send time). Accessor surface: `get_brand_asset(meta_template_name)`, `get_header_image_handle(meta_template_name, language)`, `is_known_asset_filename(filename)`. Deliberately a new module rather than an extension of v6.3.21's `whatsapp_templates.py` — the latter's single purpose is internal-event → Meta-template routing, and overloading it with brand-asset state would make the v6.3.21 contract harder to reason about. Single-purpose modules stay grep-friendly.

**Handle sidecar.** New JSON at `backend/app/services/whatsapp_template_handles.json` carries one slot per `<meta_template_name>::<language>` pair that uses a brand asset (14 slots in v6.3.23). All values start `null` until Meta approves the IMAGE-header re-submission for each pair. Committed to git on purpose — header_image_handle values are template-component IDs returned by Meta, not secrets; team visibility outweighs the marginal "spreads through git history" concern. Decision recorded in the assets README.

**Asset-serving endpoint.** New `GET /api/v1/whatsapp/assets/{filename}` on the existing `app/routers/whatsapp.py` router. Returns `FileResponse(media_type='image/png')` for filenames listed in `REGISTRY`; 404 for anything else, including FastAPI-path-conversion residue from `..` attempts. No auth — brand chrome is public, and Meta needs to reach the URL from outside our network during template submission.

**Runtime wiring.** `whatsapp_send_helper._post_template_to_meta` extended in two ways. **Mock mode:** the existing `[MOCK TEMPLATE]` log line now carries two new fields, `header_asset=<filename>` and `header_handle=<value-or-None>`. Templates with a brand asset but no handle yet (the v6.3.23 ship state for all 14 slots) also emit a `WARNING` log line naming the asset filename and `(name, language)` pair so an operator grepping production logs can see exactly which Meta approval is outstanding. **Real mode:** when the handle is non-null, the helper prepends `{type: header, parameters: [{type: image, image: {id: handle}}]}` to the Meta POST body and skips TEXT header params (the META_TEMPLATES JSON is updated to IMAGE-header form in the same submit-script run that populates the handle). When the handle is null but a brand asset exists, the helper logs the same WARNING and falls through to TEXT-header behaviour — AC `6.3.23-AC5`.

**Meta submission helper.** New `backend/scripts/submit_whatsapp_templates.py` (idempotent; `--dry-run` / `--apply`). Walks the handles JSON, skips slots whose handle is already populated (local idempotency — no GET-to-Meta round-trip per run), and either prints the would-submit plan (`--dry-run`) or POSTs each asset to Meta and writes the returned handle back to the JSON atomically via `os.replace` (`--apply`). `HttpxSubmitter.submit` is intentionally a `NotImplementedError` placeholder until Meta Business portfolio approval clears — operators run `--dry-run` for the v6.3.23 acceptance and fill in the real POST when going live. Pluggable `Submitter` Protocol allows the AC6 idempotency test to inject a stub.

**Acceptance criteria.**

(6.3.23-AC1) `REGISTRY` contains exactly 8 entries, each with a valid PNG filename matching the brand key. Verified by `test_6_3_23_ac1_registry_has_eight_template_entries`. (6.3.23-AC2) Every registry entry resolves to an asset file on disk via the helper used by the asset-serving endpoint — `test_6_3_23_ac2_each_template_resolves_to_existing_asset_file`. (6.3.23-AC3) Every asset is 640×335 px PNG under 5 MB, parametrised across all 8 brand keys via PIL — `test_6_3_23_ac3_each_asset_meets_meta_size_spec`. (6.3.23-AC4) In mock mode the `[MOCK TEMPLATE]` log line carries both the Meta template name and the brand asset filename — `test_6_3_23_ac4_mock_mode_logs_template_and_asset_name`. (6.3.23-AC5) A template with `header_image_handle = None` sends body-only with a logged `WARNING`, not an exception, in both mock and real modes — `test_6_3_23_ac5_missing_header_handle_falls_through_with_warning`. (6.3.23-AC6) `submit_whatsapp_templates.run()` reports zero new submissions on the second consecutive `--apply` run against the same Meta state — `test_6_3_23_ac6_submit_script_is_idempotent` injects a stub `Submitter` and asserts on call counts. (6.3.23-AC7) `GET /api/v1/whatsapp/assets/{filename}` returns `image/png` with the bytes on disk for whitelisted filenames; 404 otherwise — `test_6_3_23_ac7_asset_endpoint_serves_png_with_correct_content_type`. (6.3.23-AC8) All existing WhatsApp tests (v5.10 alerts, v6.3.4 briefing dispatcher, v6.3.5 invite welcome, v6.3.22 helper) continue to pass with no modification — verified by the full unit suite (1201 passing, +14 from the v6.3.22 baseline of 1187).

**Deferred (in scope for follow-up commits).** Four of the eight brand categories — `compliance_reminder`, `savings_summary`, `material_estimate`, `day7_first_insight` — ship registry entries and asset files in v6.3.23, but no caller invokes `send_with_window_decision` for them today. The brand IMAGE header activates automatically when each dispatcher lands and routes through the helper. **Two-point follow-up requirement** (recorded so future commits cannot quietly skip it):
1. Each new dispatcher commit MUST add its own AC test — `test_6_X_Y_acN_<short>_brand_header_routes` — that asserts the IMAGE component appears in the Meta POST body when the handle is populated. "The asset registry already had this entry" is **not** a substitute.
2. The same commit MUST also re-run the AC5 fall-through test for its template, confirming `header_image_handle = None` produces the body-only `WARNING` path (not an exception). Each new dispatcher reactivates the AC5 contract for its template name.

The DELIVERY_LEDGER row for v6.3.23 cross-references this requirement; the `backend/assets/whatsapp_headers/README.md` repeats it for grep-ability.

**Out of scope for v6.3.23.** `manager_checkin` and `einvoice_ready` templates (defer to v6.3.24). Per-tenant custom logos (defer indefinitely — brand consistency wins). S3/CDN hosting for assets (defer; bandwidth at current scale is trivial). Image variants per industry vertical (defer; single brand, not per-vertical). Localised headers with Hindi/Hinglish text baked into the image (defer; body carries the language, headers are visual brand only). Re-submitting v5.10 alert templates to Meta for production go-live (blocked on v5.11 portfolio review independently).

**Risks specific to v6.3.23.** (a) Until Meta approves the IMAGE-header re-submissions, all 14 handle slots stay `null` and the runtime helper logs WARNING + falls through to TEXT-header behaviour. This is the v6.3.23 ship state, not a regression — but operators reading production logs will see a steady stream of "header_image_handle not set" warnings until approval clears. Mitigated by the warning text naming exactly which `(template, language)` pair needs action and which asset filename to verify against the Meta dashboard. (b) The HEADER component on existing Meta-approved templates is currently TEXT, not IMAGE. When the submit script lands the IMAGE handle, the META_TEMPLATES JSON entry for that pair must also be updated in the same operation (HEADER component switches from `text` to `format: "IMAGE"` with no text placeholders). The helper's IMAGE-header branch assumes that invariant; if it ever drifts, the leftover TEXT header params from `_split_meta_params` are dropped (logged) rather than sent alongside the IMAGE component. (c) Pillow is now a hard backend dependency — adding it explicitly to requirements.txt rather than relying on transitive resolution closes the AC3 test stability concern.
...
| 24 | Voice notes via WhatsApp (Whisper API) | DONE v5.2-voice-notes (mock); awaiting real inbound audio | Whisper transcription + simulate-voice endpoint shipped 2026-03-30 at tag v5.2-voice-notes. Real inbound audio gated by Meta Business portfolio approval (single dependency for the WhatsApp production cutover; see §1.2 Blocked). v5.13 number unused — see §1.1 "Note on version-number gaps". | v5.2-voice-notes |
