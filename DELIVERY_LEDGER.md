...
|`message_formatters.py` style normalisation (positional + Meta HEADER reconcile)|23|v6.3.22a|in progress|— (tag pending)|(no flag — code-only)|—|n/a|tag `v6.3.22a` not yet cut; `JOB_CONFLICT_ALERT_HI` HEADER reconcile (Hindi conflict-alert is BODY-only today; will get the Devanagari "Schedule में conflict — action चाहिए" prefix when v6.3.22 lookup helper iteration adds the matching dispatcher path).|
