...
|Meta template constants wiring (full inventory)|23|v6.3.21|in progress|— (tag pending)|(no flag — code-only)|—|36/36 AC 23-AC7|tag `v6.3.21` not yet cut; dispatcher wiring of the 32 new constants (separate iteration when caller data shapes catch up); AC 23-AC3/AC4/AC5/AC6 not yet extended to the new 32 (existing parametrisations cover only the 4 v6.3.18 constants — a follow-up may broaden them); Meta API submissions for the 1 draft Hindi conflict-alert + any other unsent entries remain release-engineering work.|
|`message_formatters.py` style normalisation (positional + Meta HEADER reconcile)|23|v6.3.22a|in progress|— (tag pending)|(no flag — code-only)|—|n/a|tag `v6.3.22a` not yet cut; `JOB_CONFLICT_ALERT_HI` HEADER reconcile (Hindi conflict-alert is BODY-only today; will get the Devanagari "Schedule में conflict — action चाहिए" prefix when the matching dispatcher path lands).|
|Runtime template lookup helper (`resolve_template`) + Day-7 / give-up-nudge wiring|23 (helper); 6.3.16 + 6.4.0 (templates)|v6.3.21 part 2|in progress|— (tag pending)|(no flag — code-only)|—|25/25 coverage invariant + 12/12 helper unit + 40/40 AC 23-AC7|tag `v6.3.21` not yet cut; the 4 newly-submitted Meta templates remain `pending_meta_approval` upstream — flip to `shipped` via `scripts/merge_approved_templates.py` when WhatsApp Manager shows all four APPROVED; dispatcher wiring of `owner_day7_insight` + `engagement_give_up_nudge` callsites is a separate iteration (the helper is in place but no caller invokes these events yet).|
