...
|Consolidated push cadence (morning + evening)|6.28 v2|v6.3.19|shipped (shadow mode)|v6.3.19|`PUSH_V2_ENABLED=False` (cutover gate; shadow-mode default); `DEBUG_DISPATCH_ENABLED=True` in dev/staging (debug endpoint gate)|033 + 034|10/10 SRS §6.28.v2-AC1..AC10 (per `docs/srs_section_6_28_v2.md`)|cutover flip to `PUSH_V2_ENABLED=true` (v6.3.19.1, post 5-box gate); `dispatch_delay_alert` + `dispatch_conflict_alert` (v6.3.19.1 — legacy 8:00 + 8:30 ticks survive ungated this release per D4 scope reduction); detector test date-anchoring infrastructure fix for the 13 xfails (v6.3.19.1, see CHANGELOG note 92); Meta v2 conditional `{flag_section}`/`{next_step_section}` template re-submission (closes the §23 tone exception); legacy 5-min briefing tick code removal (v6.3.20, post shadow-verification); per-recipient locale routing once `User.language_preference` lands; `briefing_*` → `push_*` namespace unification (slice 2A-rename, future)|
|Detector test date-anchoring infrastructure fix|—|v6.3.19.1 (planned)|not started|—|(test-only — no production behaviour change)|—|0/13|Rewrite the 13 xfailed detector tests (3 idle\_machine + 2 low\_utilization + 2 manager\_silence + 2 new\_employee\_no\_show + 3 no\_progress + 1 status\_change\_alert) to use a single hardcoded TODAY/NOW anchor consistently across fixture setup AND detector invocation. Verify with `freezegun` parametrisation across a 90-day span. Strip xfail markers and `import pytest` lines added in v6.3.19 slice 2D-shadow. **Hard deadline:** if a 14th test fails for the same date-drift reason before this slice ships, treat as a release blocker and bring v6.3.19.1 forward — do NOT silently add a 14th xfail. See CHANGELOG note 92 for full context.|
