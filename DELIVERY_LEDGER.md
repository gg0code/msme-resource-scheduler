...
**Last updated:** 2026-05-14 — **v6.3.23 shipped (tagged).** Brand asset library: eight 640×335 PNG headers for WhatsApp HSM template IMAGE-header components, plus the runtime plumbing that resolves the right asset per outbound template and the operator-driven Meta submission script. New module `backend/app/services/whatsapp_template_assets.py` ships the 8-entry `REGISTRY: dict[str, BrandAsset]` + `get_brand_asset` / `get_header_image_handle` / `is_known_asset_filename` accessors; deliberately a NEW module rather than extending v6.3.21's `whatsapp_templates.py` so each module keeps a single purpose. Meta-returned handles live in committed sidecar JSON `backend/app/services/whatsapp_template_handles.json` (14 `<name>::<language>` slots, all `null` until Meta approves the IMAGE-header re-submissions). New `GET /api/v1/whatsapp/assets/{filename}` endpoint on the existing whatsapp router serves PNGs via `FileResponse(media_type='image/png')` against a strict `REGISTRY` whitelist; 404 otherwise. `whatsapp_send_helper._post_template_to_meta` extended: mock mode `[MOCK TEMPLATE]` log line now carries `header_asset=<filename>` and `header_handle=<value-or-None>`, plus a `WARNING` line whenever a brand asset exists but the handle is still null (so production-log greppers see exactly which Meta approval is outstanding). Real mode prepends `{type: header, parameters: [{type: image, image: {id: handle}}]}` to the Meta POST when the handle is non-null; otherwise falls through to TEXT header behaviour unchanged. New `backend/scripts/submit_whatsapp_templates.py` is idempotent (local handle-state check; second consecutive --apply reports zero new submissions) with `--dry-run` / `--apply` modes; the real `HttpxSubmitter.submit` POST is a `NotImplementedError` placeholder until Meta Business portfolio approval clears. Asset generator at `backend/scripts/generate_brand_headers.py` renders all 8 PNGs and SVGs from a single in-module SPEC using Pillow (no cairo dependency). Pillow added explicitly to `backend/requirements.txt`. Tests: **1201 unit-tier passing**, 3 skipped, 0 failed (+14 from v6.3.22's 1187: 7 new ACs in `tests/test_whatsapp_branded_headers.py` with AC3 parametrised across the 8 brand keys). **Migration head unchanged at 034** — invariant held end-to-end as required by the v6.3.23 brief. SRS §6.29 ACs 6.3.23-AC1..AC8 landed in the same commit set. **Deferred wiring noted (4 templates):** `compliance_reminder`, `savings_summary`, `material_estimate`, `day7_first_insight` ship registry entries + asset files but no caller invokes `send_with_window_decision` for them today; the brand IMAGE header activates automatically when each dispatcher lands and routes through the helper. Each new dispatcher commit must add its own AC test + re-run the AC5 fall-through test for its template — recorded in the SRS §6.29 stub and `backend/assets/whatsapp_headers/README.md`.

**Last updated:** 2026-05-13 — **v6.3.22 shipped (tagged).** Channel-decision send infrastructure that picks free-form vs HSM template per recipient based on Meta's 24-hour conversation window. New module `backend/app/services/whatsapp_send_helper.py` ships `send_with_window_decision()` + `SendOutcome`: reads `phone_tenant_map.last_seen_at`, branches at the 24h boundary, builds the Meta template POST body (HEADER/BODY split from `META_TEMPLATES["components"]`), and writes one of five new audit `event_type`s per outcome — `whatsapp.freeform_sent` / `whatsapp.template_sent` / `whatsapp.template_param_mismatch` / `whatsapp.window_expired_no_template` / `whatsapp.template_rejected`. Mock mode emits a new `[MOCK TEMPLATE]` breadcrumb. **No migration** — the doc's proposed migration 035 + `last_inbound_at` column was dropped after audit confirmed `last_seen_at` is written exclusively by `resolve_identity()` on inbound paths (call sites: `whatsapp.py:388/678/726`). Refactored 5 send paths through the helper: `_dispatch_one` (morning + evening) + `dispatch_delay_alert` + `dispatch_conflict_alert` in `consolidated_briefing.py`, and `send_invite_welcome` in `team_invite_whatsapp.py`. `_render_morning_message` now returns `(rendered_text, args_tuple)` so the same positional args drive both the in-window free-form `.format()` and the out-of-window `resolve_template` path. Day-7 insight dispatcher + the 5 `_send_alert` callsites in `whatsapp_alerts.py` stay on free-form for this iteration (Day-7 because each `SignalCandidate` does not yet carry the 6 positional args the `zetaops_owner_day7_insight` template expects; deferred alongside v6.4.0). Test counts: **1187 unit-tier passing**, 3 skipped, 0 failed (+8 from v6.3.21's 1179: 8 new helper unit tests in `tests/services/test_whatsapp_send_helper.py` covering the 5 channel-decision outcomes + mock-template log + content-equivalence + Hindi no-fallback). Migration head unchanged at 034. SRS §6.28.6 landed in the same commit set with ACs 6.28.6-AC1..AC8.

**Last updated:** 2026-05-13 — **v6.3.21 shipped (tagged).** Closes the Meta WhatsApp template inventory on the Python side under one tag, bundling: part 1 full-inventory wiring (32 new constants → 36 of 36 entries bound); v6.3.22a style normalisation (4 v6.3.18 constants converted from named to positional placeholders + `CONFLICT_ALERT_EN` Meta HEADER reconcile); part 2 follow-up (runtime lookup helper `resolve_template` in `app/services/whatsapp_templates.py` with 25-event `EVENT_ROUTING`, plus 4 newly-submitted Meta templates Day-7 + give-up-nudge wired with `status: pending_meta_approval` until Meta review completes); and the v6.3.18 Hindi conflict-alert transition-flag cleanup (Meta approved the entry; flag cleared, two obsolete AC 23-AC8 tests removed). Counts: **40 META_TEMPLATES, 40 bound Python constants, 25 events in EVENT_ROUTING, 4 entries with pending_meta_approval flag, 0 entries with draft_pending_meta_submission**. Tests: **1179 unit-tier passing**, 3 skipped, 0 failed (was 1108 pre-v6.3.21; net +71 across the three slices). Migration head unchanged at 034. New utility script `scripts/merge_approved_templates.py` will clear the 4 remaining `pending_meta_approval` flags after Meta approves Day-7 + give-up-nudge templates upstream. No dispatcher callsites for the 4 new templates yet — separate iteration when caller data shapes catch up.

**Last updated:** 2026-05-13 — v6.3.21 part 2 landed (untagged, in progress). Two pieces folded into the v6.3.21 scope: (1) runtime lookup helper — new module `app/services/whatsapp_templates.py` ships `resolve_template` + `EVENT_ROUTING` (25 events) + `ResolvedTemplate` + `UnknownEventError` / `ArgCountMismatchError`; new test file `tests/test_whatsapp_templates.py` with a 25-event coverage invariant + 12 unit tests covering happy paths, language fallback (hi→en_US with logged warning), error paths, str-coercion of non-string args; (2) 4 newly-submitted Meta templates wired (`zetaops_owner_day7_insight` en_US+hi for v6.3.16 First-Insight Gate; `zetaops_engagement_give_up_nudge` en_US+hi for v6.4.0 engagement ladder fallback) — 4 constants in a new Section 5 of `message_formatters.py`, 4 rows in `PYTHON_CONSTANT_BINDINGS`, 4 JSON entries with `status: pending_meta_approval`, 2 rows in `EVENT_ROUTING`. New utility `scripts/merge_approved_templates.py` clears both pending-status flags post-approval (idempotent). Counts: **40 META_TEMPLATES** (was 36), **40 bound Python constants** (was 36), **25 events** in EVENT_ROUTING (new), AC 23-AC7 audit parametrises across 40 entries. Tests: **1181 unit-tier passing**, 3 skipped, 0 failed (+41 from v6.3.22a's 1140: 4 new AC 23-AC7 + 25 coverage invariant + 12 lookup helper). Migration head unchanged at 034. **Tag pending:** v6.3.21 not yet cut.

**Last updated:** 2026-05-13 — v6.3.22a `message_formatters.py` style normalisation landed (untagged, in progress). The 4 v6.3.18 constants (`MORNING_BRIEFING_EN`, `MORNING_BRIEFING_HI`, `CONFLICT_ALERT_EN`, `DELAY_ALERT_EN`) converted from named placeholders (`{date}`, `{job_a}`, ...) to positional (`{0}, {1}, ...`) so the entire file has one consistent style across all 36 constants. Reconciled one latent v6.3.18 drift in the same pass: `CONFLICT_ALERT_EN` now includes the Meta HEADER prefix (`Schedule conflict — action needed`) that the Python constant had silently dropped — mock-mode renders + audit logs now match Meta wire output. Updated 3 production callsites in `app/services/consolidated_briefing.py` (morning EN/HI shared call at line 734, `dispatch_delay_alert` at 1697, `dispatch_conflict_alert` at 1841), 4 in `scripts/preview_v6_3_18_templates.py`, and `tests/test_message_templates.py` (4 sample dicts → tuples; 7 `.format(**sample)` → `.format(*sample)` rewrites; `_SNAP_CONFLICT_EN` updated to include the new HEADER prefix). Tests: **1140 unit-tier passing**, 3 skipped, 0 failed. Migration head unchanged at 034. **Tag pending:** v6.3.22a not yet cut; new ledger row marked `in progress` until tag lands. Unblocks the deferred v6.3.22 runtime lookup helper (its `template_string.format(*args)` strategy now works for every constant in the file).

**Last updated:** 2026-05-13 — v6.3.21 Meta template constants wiring landed (untagged, in progress). 32 new positional-placeholder template constants appended to `app/services/message_formatters.py` Section 4 (onboarding & consent, evening briefing, manager check-in, operational alerts, order intake, compliance reminders, team invite, manager engagement ladder, performance & finance). `PYTHON_CONSTANT_BINDINGS` in `app/services/whatsapp_meta_templates.py` extended from 4 → 36 entries — full coverage of the Meta inventory in `whatsapp_meta_templates.json`. AC 23-AC7 placeholder-parity audit now parametrises across all 36; 58/58 cases pass in `tests/test_message_templates.py` (was 26 with 4 wired). 107/107 pass across the related set (`test_message_formatters.py` + `test_message_templates.py` + `tests/services/test_message_dispatcher_templates.py`). The four already-wired v6.3.18 named-kwarg constants (`MORNING_BRIEFING_EN/HI`, `CONFLICT_ALERT_EN`, `DELAY_ALERT_EN`) are unchanged per spec — Section 4 uses positional `{0}, {1}, ...` slots so the file now mixes two conventions intentionally. No Meta API submissions, no dispatcher wiring, no new feature flag. Migration head unchanged at 034. **Tag pending:** v6.3.21 not yet tagged; new ledger row marked `in progress` until tag lands.

**Last updated:** 2026-05-13 — **v6.3.20 shipped (tagged).** WhatsApp natural-language pathway for top-tier owners to change push-briefing settings without leaving the chat. Three Groq AI tools (`update_push_setting` / `pause_push` / `get_push_settings`) backed by `app/services/push_settings_service.py` (whitelist + 4 validators + trilingual error templates), the existing v5.6 confirmation flow extended in `whatsapp_actions.py` (new `UPDATE_PUSH_SETTING` + `PAUSE_PUSH` ActionTypes, sync `store_pending_action_sync`, sync `_execute_*` executors), and full bridge plumbing (`actor_user_id` + `phone_number` flow from `whatsapp.py` router → `whatsapp_bridge.process_message` → `ai_service.run_ai_chat` → `execute_tool`). The two write tools refuse with `tool_requires_whatsapp_channel` from the web-UI `ai_chat` caller — the security boundary that keeps NL push-settings WhatsApp-only. Reuses migration 033 columns (`tenants.morning_sections` / `evening_sections` / `push_paused_until` from v6.3.19 slice 2A); no new schema. Briefing-classifier intercept bug closed: `whatsapp_intent.detect_briefing_request_intent` now disambiguates display intent from settings-change intent via `_SETTINGS_TIME_CUE_RE` regex + `_SETTINGS_STRONG_VERBS` frozenset. Manual WhatsApp mock-mode walkthrough at `docs/v6_3_20_manual_walkthrough.md` (9 steps) signed off green against tenant 12 before tag. Annotated tag, matching the v6.3.x series convention. Tests: **1108 unit-tier passing** (+56 from v6.3.19.1's 1052: 26 service-module + 7 plumbing + 23 disambiguation), 3 skipped, 0 failed. Migration head unchanged at 034.
...
|Meta template constants wiring (full inventory)|23|v6.3.21|shipped|v6.3.21|(no flag — code-only)|—|36/36 AC 23-AC7|dispatcher wiring of the 32 new constants (separate iteration when caller data shapes catch up); AC 23-AC3/AC4/AC5/AC6 not yet extended to the new 32 (existing parametrisations cover only the 4 v6.3.18 constants — a follow-up may broaden them).|
|`message_formatters.py` style normalisation (positional + Meta HEADER reconcile)|23|v6.3.22a|shipped|v6.3.21 (bundled)|(no flag — code-only)|—|n/a|`JOB_CONFLICT_ALERT_HI` HEADER reconcile (Hindi conflict-alert is BODY-only today; will get the Devanagari "Schedule में conflict — action चाहिए" prefix when the matching dispatcher path lands).|
|Runtime template lookup helper (`resolve_template`) + Day-7 / give-up-nudge wiring|23 (helper); 6.3.16 + 6.4.0 (templates)|v6.3.21 part 2|shipped|v6.3.21|(no flag — code-only)|—|25/25 coverage invariant + 12/12 helper unit + 40/40 AC 23-AC7|the 4 newly-submitted Meta templates remain `pending_meta_approval` upstream — flip via `scripts/merge_approved_templates.py` when WhatsApp Manager shows all four APPROVED; dispatcher wiring of `owner_day7_insight` + `engagement_give_up_nudge` callsites is a separate iteration (the helper is in place but no caller invokes these events yet).|
|WhatsApp brand asset library (IMAGE-header chrome)|6.29|v6.3.23|shipped|v6.3.23|(no flag — code-only)|—|8/8 (6.3.23-AC1..AC8)|14 `<meta_template>::<language>` handle slots remain `null` pending Meta IMAGE-header re-submission approvals — operator runs `scripts/submit_whatsapp_templates.py --apply` after Meta Business portfolio review clears. Dispatcher wiring for 4 of 8 brand keys deferred (`compliance_reminder`, `savings_summary`, `material_estimate`, `day7_first_insight`) — registry rows + asset files ship now, but no caller routes through `send_with_window_decision` for them today; brand header activates automatically when each dispatcher lands. Each new dispatcher commit MUST add its own AC test + re-run the AC5 null-handle fall-through test for its template.|
